...
data.forEach((q, index) => {
  ...
  if (q.question) {
    if (questions.has(q.question)) {
      errors.push(`[Index ${index}] Duplicate question: ${q.question.substring(0, 20)}`);
    }
...
console.log(`Result for ${file}: ${errors.length} errors found.`);
